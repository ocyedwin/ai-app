...
class VideoIndexJobTest < ActiveJob::TestCase
  ...
  test "processes video and search manually" do
    VideoIndexJob.perform_now(@video)
    assert_performed_jobs 1, only: VideoIndexSearchJob do
      VideoIndexSearchJob.perform_later(@video)
    end
  end
end
